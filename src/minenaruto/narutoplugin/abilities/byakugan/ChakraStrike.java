...
import java.util.*;
public class ChakraStrike extends AbilitiesMain {
    private Item item = new Item(Material.DIAMOND_HOE, 74, "§7[§6Naruto§7] §fУдар чакры", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));

    @Override
    public void RightClick(Player player, NarutoPlayer pl) {
        if (AbilityListener.checkChakraItem(player, getItem().getName(), 25, 0, 0, 0, 0)) {
            Location loc = player.getLocation().clone();
            createChakraStrike(loc, player);
        }
    }

    public void createChakraStrike(Location loc, Player player) {
