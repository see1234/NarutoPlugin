...
public class WaterShark extends AbilitiesMain {
    private minenaruto.narutoplugin.iditems.Item item = new minenaruto.narutoplugin.iditems.Item(Material.DIAMOND_HOE, 71, "§7[§6Naruto§7] §bАкула-Ракета", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));
                ...
                if (distance.containsKey(stand) && distance.containsValue(p))
                    if (p.getLocation().distance(stand.getLocation()) < Distance) {
                        ...
                        for(Entity en : stand.getNearbyEntities(1,1,1)) {
                            if(!en.equals(stand) && !en.equals(p)) {
                                AbilityListener.damageEntity(en, p, 4);
                            }
                        }
                        if (stand.getEyeLocation().getBlock().getType() != Material.AIR && !stand.getEyeLocation().getBlock().getType().name().endsWith("PLANT") && stand.getEyeLocation().getBlock().getType() != Material.GRASS_BLOCK && stand.getEyeLocation().getBlock().getType().name().endsWith("_GRASS")) {
