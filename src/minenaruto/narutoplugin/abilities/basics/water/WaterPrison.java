...
public class WaterPrison extends AbilitiesMain {
    private Item item = new Item(Material.DIAMOND_HOE, 71, "§7[§6Naruto§7] §bВодяная тюрьма", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));

    @Override
    public void RightClick(Player player, NarutoPlayer pl) {
        if (AbilityListener.checkChakraItem(player, getItem().getName(), 20, 0, 0, 0, 0)) {
            Location loc = player.getLocation().clone();
            createWaterPrison(loc, player);
        }
    }

    public void createWaterPrison(Location loc, Player player) {
            ...
            public void run() {
                if (count++ != 50) {
                    for (Entity en : loc.getWorld().getNearbyEntities(loc, 3, 3, 3)) {
                        ...
                        en.setVelocity(new Vector(0, 0.1, 0)); // Поднимает врагов вверх
