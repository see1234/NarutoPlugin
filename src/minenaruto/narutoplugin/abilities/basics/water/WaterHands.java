...
public class WaterHands extends AbilitiesMain {
    private minenaruto.narutoplugin.iditems.Item item = new minenaruto.narutoplugin.iditems.Item(Material.DIAMOND_HOE, 64, "§7[§6Naruto§7] §bВодные руки", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));
                        ...
                        if (entity != player) {
                            if (entity instanceof Player) {
                                if (hasPvpZone(entity)) {
                                    addDamageEntity(player, (Player)entity,3);
                            ...
                            if (entity instanceof LivingEntity) {
                                entity.setVelocity(block.getLocation().getDirection().add(new Vector(0, 1, 0)));
                                addDamageEntity(player,entity, 3);
                            }
                        }
                    }
                } else {
