...
public class StoneBroke extends AbilitiesMain {
    private Item item = new Item(Material.DIAMOND_HOE, 59, "§7[§6Naruto§7] §6Разрыв земли", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));
            ...
            for (; x <= loc.getBlockX() + radius; x++) {
                for (int y = loc.getBlockY() - 2; y <= loc.getBlockY(); y++) {
                    for (int z = loc.getBlockZ() - radius; z <= loc.getBlockZ() + radius; z++) {
                        ...
                        if ((l.getBlock().getType().equals(Material.DIRT) || l.getBlock().getType().equals(Material.GRASS_BLOCK) || l.getBlock().getType().equals(Material.SAND) || l
