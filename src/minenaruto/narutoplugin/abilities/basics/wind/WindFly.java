...
public class WindFly extends AbilitiesMain {
    private minenaruto.narutoplugin.iditems.Item item = new minenaruto.narutoplugin.iditems.Item(Material.DIAMOND_HOE, 65, "§7[§6Naruto§7] §2Ветреный полет", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));
    @Override
    ...
    public void runTaskAbility(Player player) {
        BukkitRunnable task = new BukkitRunnable() {
            ...
            public void run() {
                if (count++ != 200) {
                    player.setVelocity(player.getEyeLocation().getDirection().multiply(0.1));

                    ParticleEffect.CLOUD.display(player.getLocation(), 10, 0.3, 0.3, 0.3);
