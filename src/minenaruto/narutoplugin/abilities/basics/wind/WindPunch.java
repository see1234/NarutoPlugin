...
public class WindPunch extends AbilitiesMain {
    private minenaruto.narutoplugin.iditems.Item item = new minenaruto.narutoplugin.iditems.Item(Material.DIAMOND_HOE, 65, "§7[§6Naruto§7] §2Ветреный удар", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));
    ...
    public  void abilityStart(Player player ) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    loc.getWorld().spawnParticle(Particle.REDSTONE, loc.getX(), loc.getY(), loc.getZ(), 8, 0, 0, 0, dust);

                    for (Entity en : loc.getWorld().getNearbyEntities(loc,1,1,1)) {
                       if(en instanceof LivingEntity) {
                           if (AbilityListener.hasPvpZone(en)) {
                               AbilityListener.damageEntity(en, player, 3);
                           }
                       }
