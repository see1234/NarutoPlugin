...
public class BanshoTenin extends AbilitiesMain {
    private Item item = new Item(Material.DIAMOND_HOE, 77, "§7[§6Naruto§7] §5Отталкивание", List.of("§7Использование:§f ПКМ;§7Получение новой способки:§f ПКМ+ШИФТ".split(";")));

    @Override
    public void RightClick(Player player, NarutoPlayer pl) {
        if (AbilityListener.checkChakraItem(player, getItem().getName(), 30, 0, 0, 0, 0)) {
            Location loc = player.getLocation().clone();
            createBanshoTenin(loc, player);
        }
    }

    public void createBanshoTenin(Location loc, Player player) {
